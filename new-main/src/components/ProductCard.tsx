...
import { Eye, Plus, Clock, Sparkles, Check, Truck, Lock } from 'lucide-react';
...
interface ProductCardProps {
  ...
  onOpenAdminSpace?: (slotNumber?: number) => void;
}
  ...
  return (
      ...
      >
        ...
        <div className="relative aspect-[4/5] w-full overflow-hidden bg-[#faf8f5] p-2">
          ...
          <div className="absolute top-3.5 left-3.5 right-3.5 flex items-center justify-between gap-1.5 z-10 pointer-events-none">
            ...
            <div className="flex items-center gap-1 pointer-events-auto">
              {onOpenAdminSpace && (
                <button
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    onOpenAdminSpace(product.slotNumber);
                  }}
                  title="Open Space in Admin (Password Protected)"
                  className="px-2 py-0.5 rounded-full bg-stone-900/85 hover:bg-stone-950 text-[#e5c07b] text-[9px] font-mono tracking-wider flex items-center gap-1 border border-[#946e1c]/40 cursor-pointer backdrop-blur-md transition-all shadow-xs hover:scale-105"
                >
                  <Lock className="w-2.5 h-2.5 text-[#e5c07b]" />
                  <span>Admin</span>
                </button>
              )}
