...
export default function App() {
  ...
  const [pendingAdminAction, setPendingAdminAction] = useState<'bulk' | 'manager' | null>(null);
    ...
    <div className="min-h-screen bg-[#fdfcfb] text-stone-900 flex flex-col selection:bg-[#946e1c] selection:text-white">
        ...
        <div className="mb-8 p-4 sm:p-5 rounded-2xl bg-gradient-to-r from-[#faf8f4] to-[#f4efe3] border border-[#d8cfbe] shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2.5 shrink-0">
            <input
              ...
              className="hidden"
              onChange={(e) => {
                handleDirectBulkImport(e.target.files);
            ...
            <button
              ...
              onClick={() => {
                if (!isAdminUnlocked()) {
                  setPendingAdminAction('bulk');
                  setIsUploadAuthOpen(true);
                  return;
                }
                bulkInputRef.current?.click();
              }}
              className="px-4 py-2 bg-stone-900 hover:bg-black text-white text-xs font-medium rounded-xl transition-colors cursor-pointer flex items-center gap-1.5 shadow-sm"
            >
      ...
      <AdminPasswordModal
        ...
        onSuccess={() => {
          ...
          setIsUploadAuthOpen(false);
          if (action === 'bulk') {
            setTimeout(() => bulkInputRef.current?.click(), 0);
          } else if (action === 'manager') {
            setIsAssetManagerOpen(true);
          }
        }}
        title="Admin Authorization Required"
        description="Enter the admin password before attaching or managing original portfolio files."
      />
